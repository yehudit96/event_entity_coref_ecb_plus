...
def test_significance():
    ...
    for i in range(1000):
        ...
        chirps_sort = sorted(selected, key=lambda r:r[1], reverese=True)
        clf_sort = sorted(selected, key=lambda r:r[2], reverese=True)
        chirps_AP = AP(map(lambda r: r[0], chirps_sort))
        clf_AP = AP(map(lambda r: r[0], clf_sort))
        chirps_scores.append(chirps_AP)
        clf_scores.append(clf_AP)
    if not os.path.exists(args.out_dir):
        os.mkdir(args.out_dir)
    with open(os.path.join(arg.out_dir, 'a_scores.txt'), 'wb') as f_a:
        f_a.write('\n'.join(chirps_AP))
    with open(os.path.join(arg.out_dir, 'b_scores.txt'), 'wb') as f_b:
        f_b.write('\n'.join(clf_AP))
